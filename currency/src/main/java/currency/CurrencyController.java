...
public class CurrencyController {
	...
	public String conversion(@PathVariable("accountNum") Integer id, Model model) {
		...
		repository.findById((long) id).ifPresent(client -> {		
		
		double euroValue = client.getAccountValue() * euroRate;
		double cadValue = client.getAccountValue() * cadRate;
		
		model.addAttribute("EUR", euroValue);
		model.addAttribute("CAD", cadValue);
		model.addAttribute("firstName", client.getFirstName());
		model.addAttribute("lastName", client.getLastName());
		model.addAttribute("id", client.getAccountNum());
